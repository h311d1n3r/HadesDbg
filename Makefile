...
release: bin/lib
	...
	@$(CC) $(HADESDBG_SRC)/*.cpp -I$(HADESDBG_INC) -I$(ASMJIT_INC) -I$(SIMPLESON_INC) -o$(HADESDBG_RELEASE_OUT)/$(HADESDBG_32_NAME) $(COMPILE_FLAGS) $(COMPILE_32_FLAGS) -L"$(HADESDBG_OUT_LIB)/$(LIB_32_DIR)" -l$(ASMJIT_NAME) -l$(SIMPLESON_NAME)
	@chmod 777 $(HADESDBG_RELEASE_OUT)/$(HADESDBG_32_NAME)
	@echo "Releasing 64bit version..."
	@$(CC) $(HADESDBG_SRC)/*.cpp -I$(HADESDBG_INC) -I$(ASMJIT_INC) -I$(SIMPLESON_INC) -o$(HADESDBG_RELEASE_OUT)/$(HADESDBG_64_NAME) $(COMPILE_FLAGS) $(COMPILE_64_FLAGS) -L"$(HADESDBG_OUT_LIB)/$(LIB_64_DIR)" -l$(ASMJIT_NAME) -l$(SIMPLESON_NAME)
